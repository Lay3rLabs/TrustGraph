...
contract DeployZodiacSafes is Common {
     ...
     */
    function run(string calldata serviceManagerAddr, string calldata merkleSnapshotAddr) public {
        ...
        SafeDeployment memory safe1 = deployZodiacSafeWithSignerAndMerkle(
            safeSingleton, safeFactory, deployer, serviceManager, merkleSnapshot, "Safe1"
        );
    ...
    function generateSignature(address signer, GnosisSafe, /*_safe*/ address, /*to*/ bytes memory /*data*/ )
        internal
        pure
    ...
    function writeDeploymentResults(
        ...
        safe1Json.serialize("merkle_gov_module", Strings.toChecksumHexString(safe1.merkleGovModule));
        safe1Json.serialize("signer_module", Strings.toChecksumHexString(safe1.signerModule));
        safe1Json.serialize("threshold", safe1.threshold);
        vm.serializeBool(safe1Json, "modules_enabled", safe1.modulesEnabled);
        safe1Json = vm.serializeUint(safe1Json, "funding_amount", safe1.fundingAmount);

        // Safe 2 data
        string memory safe2Json = "safe2";
        safe2Json.serialize("address", Strings.toChecksumHexString(safe2.safe));
        safe2Json.serialize("wavs_module", Strings.toChecksumHexString(safe2.wavsModule));
        vm.serializeUint(safe2Json, "threshold", safe2.threshold);
        vm.serializeBool(safe2Json, "modules_enabled", safe2.modulesEnabled);
        safe2Json = vm.serializeUint(safe2Json, "funding_amount", safe2.fundingAmount);
