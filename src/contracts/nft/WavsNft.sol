    ...
    IWavsServiceHandler
{
    ...
    function handleSignedEnvelope(Envelope calldata envelope, SignatureData calldata signatureData) external {
        ...
        IWavsNftServiceTypes.WavsResponse memory wavsResponse =
            abi.decode(envelope.payload, (IWavsNftServiceTypes.WavsResponse));

        // Handle the mint or update by checking the trigger type
        if (wavsResponse.wavsTriggerType == IWavsNftServiceTypes.WavsTriggerType.MINT) {
        ...
        } else if (wavsResponse.wavsTriggerType == IWavsNftServiceTypes.WavsTriggerType.UPDATE) {
            // Decode the update info
            IWavsNftServiceTypes.WavsUpdateResult memory updateResult =
                abi.decode(wavsResponse.data, (IWavsNftServiceTypes.WavsUpdateResult));

            // Update the tokenURI
            _setTokenURI(updateResult.tokenId, updateResult.tokenURI);

            // Check if the trigger exists and is not already fulfilled
            require(receipts[updateResult.triggerId].updater != address(0), "Trigger does not exist");
            require(!receipts[updateResult.triggerId].fulfilled, "Trigger already fulfilled");

            // Mark the trigger as fulfilled
            receipts[updateResult.triggerId].fulfilled = true;

            // Send the update fee to the funds recipient
            (bool success,) = payable(fundsRecipient).call{value: receipts[updateResult.triggerId].updateFee}("");
     ...
     */
    function setUpdateFee(uint256 newFee) external onlyRole(DEFAULT_ADMIN_ROLE) {
        updateFee = newFee;
    }
     ...
     */
    function setFundsRecipient(address newRecipient) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function triggerUpdate(uint256 tokenId, string calldata prompt) external payable {
        ...
        nextTriggerId = IWavsNftServiceTypes.TriggerId.wrap(IWavsNftServiceTypes.TriggerId.unwrap(nextTriggerId) + 1);

        // Store metadata for this update request
        receipts[nextTriggerId] = UpdateReceipt({updater: msg.sender, fulfilled: false, updateFee: updateFee});
