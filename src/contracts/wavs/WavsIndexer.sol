...
contract WavsIndexer is IWavsServiceHandler, IWavsIndexer, Semver {
    ...
    mapping(string => mapping(address => bytes32[])) public eventsByChainIdAndContract;
    ...
    mapping(string => mapping(address => mapping(address => bytes32[]))) public eventsByChainIdAndContractAndAddress;
    mapping(string => mapping(string => bytes32[])) public eventsByTypeAndTag;
    mapping(address => mapping(string => bytes32[])) public eventsByAddressAndType;
    mapping(address => mapping(string => bytes32[])) public eventsByAddressAndTag;
    mapping(string => mapping(address => mapping(string => bytes32[]))) public eventsByChainIdAndContractAndType;
    mapping(string => mapping(address => mapping(string => bytes32[]))) public eventsByChainIdAndContractAndTag;
    mapping(address => mapping(string => mapping(string => bytes32[]))) public eventsByAddressAndTypeAndTag;
    mapping(string => mapping(address => mapping(string => mapping(string => bytes32[])))) public
        eventsByChainIdAndContractAndTypeAndTag;
    ...
    function handleSignedEnvelope(
        ...
        try this.decodeIndexingPayload(envelope.payload) returns (IndexingPayload memory decodedPayload) {
    ...
    function decodeIndexingPayload(bytes calldata payload) external pure returns (IndexingPayload memory) {
    ...
    function _indexEvent(bytes32 eventId, IndexedEvent memory event_) internal {
        ...
        eventsByChainIdAndContract[event_.chainId][event_.relevantContract].push(eventId);
        eventsByType[event_.eventType].push(eventId);
        eventsByChainIdAndContractAndType[event_.chainId][event_.relevantContract][event_.eventType].push(eventId);
        ...
        for (uint256 i = 0; i < event_.tags.length; i = uncheckedInc(i)) {
            ...
            eventsByChainIdAndContractAndTag[event_.chainId][event_.relevantContract][tag].push(eventId);
            eventsByChainIdAndContractAndTypeAndTag[event_.chainId][event_.relevantContract][event_.eventType][tag].push(
                eventId
            );
        }

        // Index by relevant addresses
        for (uint256 i = 0; i < event_.relevantAddresses.length; i = uncheckedInc(i)) {
            ...
            eventsByChainIdAndContractAndAddress[event_.chainId][event_.relevantContract][addr].push(eventId);
            ...
            for (uint256 j = 0; j < event_.tags.length; j = uncheckedInc(j)) {
                ...
                eventsByAddressAndTypeAndTag[addr][event_.eventType][event_.tags[j]].push(eventId);
            }
        }

        emit EventIndexed(eventId, event_.relevantContract, event_.eventType, event_.relevantAddresses, event_.tags);
    }

    /// @notice Indexes multiple events in a batch
    /// @param events_ Array of events to index
    function _batchIndexEvents(bytes20 baseEventId, IndexedEvent[] memory events_) internal {
        for (uint256 i = 0; i < events_.length; i = uncheckedInc(i)) {
            // concat 20-byte eventId with 12-byte index
            bytes32 eventId = bytes32(abi.encodePacked(baseEventId, bytes12(uint96(i))));
    ...
    {
        bytes32[] memory eventIds = _sliceUIDs(eventsByChainId[chainId], start, length, reverseOrder);
    ...
    function getEventsByContract(
        ...
        bytes32[] memory eventIds =
            _sliceUIDs(eventsByChainIdAndContract[chainId][contract_], start, length, reverseOrder);
    ...
    function getEventsByAddress(address addr, uint256 start, uint256 length, bool reverseOrder)
        ...
        returns (IndexedEvent[] memory)
    {
        bytes32[] memory eventIds = _sliceUIDs(eventsByAddress[addr], start, length, reverseOrder);
    ...
    function getEventsByType(string calldata eventType, uint256 start, uint256 length, bool reverseOrder)
        ...
        returns (IndexedEvent[] memory)
    {
        bytes32[] memory eventIds = _sliceUIDs(eventsByType[eventType], start, length, reverseOrder);
    ...
    function getEventsByTag(string calldata tag, uint256 start, uint256 length, bool reverseOrder)
        ...
        returns (IndexedEvent[] memory)
    {
        bytes32[] memory eventIds = _sliceUIDs(eventsByTag[tag], start, length, reverseOrder);
    ...
    function getEventsByContractAndAddress(
        ...
        bytes32[] memory eventIds =
            _sliceUIDs(eventsByChainIdAndContractAndAddress[chainId][contract_][addr], start, length, reverseOrder);
    ...
    function getEventsByTypeAndTag(
        ...
        bytes32[] memory eventIds = _sliceUIDs(eventsByTypeAndTag[eventType][tag], start, length, reverseOrder);
    ...
    function getEventsByAddressAndType(
        ...
        bytes32[] memory eventIds = _sliceUIDs(eventsByAddressAndType[addr][eventType], start, length, reverseOrder);
    ...
    /// @param reverseOrder Whether to return in reverse chronological order
    /// @return Array of IndexedEvent structs
    function getEventsByAddressAndTag(address addr, string memory tag, uint256 start, uint256 length, bool reverseOrder)
        external
        view
        returns (IndexedEvent[] memory)
    {
        bytes32[] memory eventIds = _sliceUIDs(eventsByAddressAndTag[addr][tag], start, length, reverseOrder);
    ...
    function getEventsByContractAndType(
        ...
        bytes32[] memory eventIds =
            _sliceUIDs(eventsByChainIdAndContractAndType[chainId][contract_][eventType], start, length, reverseOrder);
    ...
    function getEventsByContractAndTag(
        ...
        bytes32[] memory eventIds =
            _sliceUIDs(eventsByChainIdAndContractAndTag[chainId][contract_][tag], start, length, reverseOrder);
    ...
    function getEventsByAddressAndTypeAndTag(
        ...
        bytes32[] memory eventIds =
            _sliceUIDs(eventsByAddressAndTypeAndTag[addr][eventType][tag], start, length, reverseOrder);
    ...
    function getEventsByContractAndTypeAndTag(
        ...
        bytes32[] memory eventIds = _sliceUIDs(
            eventsByChainIdAndContractAndTypeAndTag[chainId][contract_][eventType][tag], start, length, reverseOrder
        );
    ...
    /// @notice Gets total number of events by contract
    function getEventCountByAddress(address addr) external view returns (uint256) {
    ...
    function getEventCountByContractAndAddress(string calldata chainId, address contract_, address addr)
        ...
        returns (uint256)
    {
        return eventsByChainIdAndContractAndAddress[chainId][contract_][addr].length;
    }
    ...
    function getEventCountByContractAndType(string calldata chainId, address contract_, string calldata eventType)
        ...
        returns (uint256)
    {
        return eventsByChainIdAndContractAndType[chainId][contract_][eventType].length;
    }
    ...
    function getEventCountByContractAndTypeAndTag(
        ...
        return eventsByChainIdAndContractAndTypeAndTag[chainId][contract_][eventType][tag].length;
    }
        ...
        returns (bytes32[] memory)
    {
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < len; ++i) {
                res[i] = uids[reverseOrder ? uidsLength - (start + i + 1) : start + i];
            }
    ...
    function _getEventsByIds(bytes32[] memory eventIds) internal view returns (IndexedEvent[] memory) {
    ...
    function _generateEventSummary(IndexedEvent memory event_) internal pure returns (string memory) {
