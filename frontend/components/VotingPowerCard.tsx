...
export function VotingPowerCard({
  ...
  // Debug logging
  console.log("VotingPowerCard props:", {
    userVotingPower,
    proposalThreshold,
    canCreateProposal,
    merkleData: merkleData ? "present" : "null",
    hasVotingPower,
  });
    ...
    <div className="border border-gray-700 bg-black/10 p-6 rounded-sm space-y-4">
      ...
      {isLoading && (
        <div className="text-center py-4">
          <div className="terminal-bright text-sm">◉ LOADING VOTING POWER ◉</div>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <div className={`text-2xl ${hasVotingPower ? 'terminal-bright' : 'terminal-dim'}`}>
                {userVotingPower ? formatVotingPower(userVotingPower) : "0"}
              </div>
              <div className={`text-xs ${hasVotingPower ? 'text-green-400' : 'system-message'}`}>
            ...
            <div className="space-y-2">
              ...
              <div className={`text-xs ${canCreateProposal ? 'text-green-400' : 'text-yellow-400'}`}>
                {canCreateProposal ? '✓ Can create proposals' : '◯ Below proposal threshold'}
              </div>
            </div>
          </div>
          ...
          {/* Voting Power Status */}
          <div className="border-t border-gray-700 pt-4 space-y-3">
            <div className="terminal-bright text-sm">GOVERNANCE PARTICIPATION</div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div className="space-y-2">
                <div className="terminal-dim text-xs">VOTING ELIGIBILITY</div>
                <div className={`${hasVotingPower ? 'text-green-400' : 'terminal-dim'}`}>
              ...
              <div className="space-y-2">
                ...
                <div className={`${canCreateProposal ? 'text-green-400' : 'terminal-dim'}`}>
              ...
              <div className="space-y-2">
                ...
                <div className="bg-gray-700 h-2 rounded">
                  <div
                    ...
                    style={{
                      width: `${Math.min(
                        (Number(userVotingPower) / Number(proposalThreshold)) * 100,
                        100
                      )}%`,
                    }}
                  />
                ...
                <div className="terminal-dim text-xs text-center">
                  {((Number(userVotingPower) / Number(proposalThreshold)) * 100).toFixed(1)}%
          ...
          {!hasVotingPower && (
            <div className="border-t border-gray-700 pt-4 space-y-3">
              <div className="terminal-bright text-sm">HOW TO GET VOTING POWER</div>
              <div className="space-y-2 text-sm terminal-dim">
                <div>• Participate in attestation activities to earn governance tokens</div>
                <div>• Claim your rewards from the rewards distribution system</div>
                <div>• Voting power is determined by your token balance in the merkle tree</div>
                <div>• Check the rewards page to see if you have unclaimed tokens</div>
              </div>
            </div>
          )}
        </div>
