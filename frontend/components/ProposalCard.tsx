...
import { ProposalCore, ProposalAction, ProposalState, VoteType } from "@/hooks/useGovernance";
...
export function ProposalCard({
  ...
  const handleVote = useCallback(async (support: VoteType) => {
    ...
    try {
      ...
      if (hash) {
        const voteText = support === VoteType.For ? "FOR" : support === VoteType.Against ? "AGAINST" : "ABSTAIN";
      ...
      <div className="space-y-2">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-1">
            <div className="ascii-art-title text-lg">PROPOSAL #{proposalId}</div>
            <div className="terminal-dim text-xs">
              Proposer: {proposal.proposer.slice(0, 10)}...{proposal.proposer.slice(-8)}
            </div>
          </div>
          <div className="flex items-center space-x-4">
            <div className={`text-xs px-2 py-1 rounded border ${
              isActive ? 'border-green-700 bg-green-900/20 text-green-400' :
              state === ProposalState.Succeeded ? 'border-blue-700 bg-blue-900/20 text-blue-400' :
              state === ProposalState.Executed ? 'border-purple-700 bg-purple-900/20 text-purple-400' :
      ...
      {actions.length > 0 && (
        <div className="border-t border-gray-700 pt-4 space-y-3">
          ...
          {actions.map((action, index) => (
            <div key={index} className="border border-gray-700 p-3 rounded-sm space-y-2">
              ...
              {action.description && (
                <div className="space-y-1">
                  ...
                  <div className="terminal-text text-sm">{action.description}</div>
                </div>
              )}
              {action.data !== "0x" && (
                <div className="space-y-1">
        ...
        <div className="grid grid-cols-3 gap-4 text-xs">
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="terminal-text">{formatVotingPower(proposal.forVotes.toString())}</span>
            </div>
            ...
            <div className="terminal-dim text-center">{forPercentage.toFixed(1)}%</div>
          </div>
          <div className="space-y-2">
            <div className="flex justify-between">
              <span className="terminal-dim">AGAINST</span>
              <span className="terminal-text">{formatVotingPower(proposal.againstVotes.toString())}</span>
            </div>
            <div className="bg-gray-700 h-2 rounded">
              <div
            ...
            <div className="terminal-dim text-center">{againstPercentage.toFixed(1)}%</div>
          </div>
          <div className="space-y-2">
            <div className="flex justify-between">
              <span className="terminal-dim">ABSTAIN</span>
              <span className="terminal-text">{formatVotingPower(proposal.abstainVotes.toString())}</span>
            </div>
            <div className="bg-gray-700 h-2 rounded">
              <div
            ...
            <div className="terminal-dim text-center">{abstainPercentage.toFixed(1)}%</div>
          </div>
        </div>
