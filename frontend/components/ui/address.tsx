  ...
  const renderText = () => {
    switch (displayMode) {
      ...
      default:
        return (
          <>
            <span className={cn(textClasses, 'hidden sm:inline')}>
              {displayedText}
            </span>
            <span className={cn(textClasses, 'sm:hidden')}>
              {truncatedText}
            </span>
          </>
        )
  ...
  const content = (
    ...
    >
      ...
      {showCopyIcon && !isLoading && (
        <button
          onClick={handleCopy}
          className="opacity-0 group-hover:opacity-100 transition-opacity focus:opacity-100 focus:outline-none p-0.5 rounded hover:bg-gray-100 dark:hover:bg-gray-800"
          tabIndex={0}
