...
export default function IncentivesPage() {
  ...
  const filteredCategories = incentiveCategories.filter(category => 
  ...
  const totalPossibleRewards = incentiveCategories.reduce((sum, cat) => sum + cat.totalRewards, 0);
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        <div className="terminal-text text-sm">
          Earn $EN0 tokens by contributing to ecosystem growth across key performance metrics.
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-black/20 border border-gray-700 p-4 rounded-sm">
          <div className="terminal-bright text-lg">{totalPossibleRewards.toLocaleString()}</div>
          <div className="terminal-dim text-xs">TOTAL $EN0 POOL</div>
        </div>
        <div className="bg-black/20 border border-gray-700 p-4 rounded-sm">
          <div className="terminal-bright text-lg">{incentiveCategories.length}</div>
          <div className="terminal-dim text-xs">PROGRAM TRACKS</div>
        </div>
        <div className="bg-black/20 border border-gray-700 p-4 rounded-sm">
          <div className="terminal-bright text-lg">
            {incentiveCategories.reduce((sum, cat) => sum + cat.metrics.length, 0)}
          </div>
      ...
      <div className="space-y-8">
        {filteredCategories.map((category) => (
          <div key={category.id} className="border border-gray-700 bg-black/10 rounded-sm">
            {/* Category Header */}
            <div className="p-6 border-b border-gray-700">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h2 className="terminal-command text-xl">{category.title}</h2>
                    <p className="terminal-text text-sm mt-1">{category.description}</p>
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-center space-x-3">
                        <span className={`text-lg ${getTierColor(metric.tier)}`}>
                          {getTierIcon(metric.tier)}
                        </span>
                        <div>
                          <h3 className="terminal-bright text-base">{metric.name}</h3>
                          <p className="terminal-text text-sm">{metric.description}</p>
                          <div className={`text-xs mt-1 ${getTierColor(metric.tier)}`}>
                            {metric.tier.toUpperCase()} TIER
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="terminal-bright text-lg">{metric.reward}</div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-xs">
                        <span className="terminal-dim">
                          {getProgressPercentage(metric.currentValue, metric.targetValue).toFixed(1)}% Complete
                        </span>
                        <span className="terminal-dim">
                          {metric.targetValue - metric.currentValue > 0 
                            ? `${(metric.targetValue - metric.currentValue).toFixed(metric.unit === "%" ? 1 : 0)} ${metric.unit} remaining`
                            : "Target achieved!"
                          }
                        </span>
                      </div>
        ...
        <div className="bg-black/20 border border-gray-700 p-6 rounded-sm">
          <div className="text-center space-y-4">
            ...
            <div className="terminal-text text-sm max-w-2xl mx-auto">
              Choose your path and begin earning $EN0 tokens. Each metric is tracked automatically,
