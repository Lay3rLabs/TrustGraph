...
export function useOperators() {
  ...
  // Get operators data
  const { data: operatorsData, isLoading, error } = useReadContract({
    address: poaServiceManagerAddress,
    abi: [{
      inputs: [
  ...
  const operators = useQuery({
    queryKey: ['operators', operatorsData ? { addresses: operatorsData[0], weights: operatorsData[1].map(w => w.toString()) } : null],
