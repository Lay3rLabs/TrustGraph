...
import { indexerConfig, easConfig } from "@/lib/contracts";
...
function useSchemaAttestationUIDs(schemaUID: string, limit = 10, totalCount?: number) {
  const { data: rawUIDs, error, isLoading } = useReadContract({
    ...indexerConfig,
...
export function useIndividualAttestation(uid: string) {
  const { data: rawData, error, isLoading } = useReadContract({
    ...easConfig,
