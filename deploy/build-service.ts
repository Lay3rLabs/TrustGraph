...
const main = async () => {
  ...
  for (const component of expandedComponents) {
    ...
    if ('block_interval' in component.trigger) {
      ...
      console.log(
        chalk.greenBright(
          `  Trigger cron: ${component.trigger.cron.schedule} from ${new Date(Number(component.trigger.cron.start_time) / 1e6).toISOString()} to ${new Date(Number(component.trigger.cron.end_time) / 1e6).toISOString()}`
        )
      )
